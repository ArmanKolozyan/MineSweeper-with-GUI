...
void remove_flag(struct cell *the_cell, struct flags_info *flags_info, int total_bombs) {
...
void place_flag(struct cell *the_cell, struct flags_info *flags_info, int total_bombs) {
    if (!the_cell->flagged) {
        ...
        remove_flag(the_cell, flags_info, total_bombs); // if the cell was already flagged, the player gets his flag back
    }
}
...
*/
void reveal_neighbours(struct game_board *game_board, int row, int column, struct flags_info *flags_info, int total_bombs) {
    for (int off_i = -1; off_i <= 1; off_i++) {
        for (int off_j = -1; off_j <= 1; off_j++) { // checking all the neighbours
            ...
            }
            {
                ...
                {
                    reveal(game_board, neighbour_i, neighbour_j, flags_info, total_bombs);
                }
            }
        }
    }
...
void reveal(struct game_board *game_board, int row, int column, struct flags_info *flags_info, int total_bombs) {
    ...
    if (the_cell->flagged) {
        remove_flag(the_cell, flags_info, total_bombs); // personal choice: if the player wants to reveal a flagged cell, the flag is given back to the player before the reveal
    }
    if (the_cell->bomb) {
        GAME_OVER = 1;
        ...
        if (the_cell->neighbours_count == 0) {
            reveal_neighbours(game_board, row, column, flags_info, total_bombs);
        }
    }
}